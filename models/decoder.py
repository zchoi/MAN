...
import ipdb
from .encoder import DecoderLayer
...
class Decoder(nn.Module):
    def __init__(self, opt, filed):
        ...
        if opt.attention == 'soft':
            ...
            self.att_text = SoftAttention(self.hidden_size, self.hidden_size, self.att_size)
    ...
    def init_weights(self):
        if 'msvd' in self.dataset or 'vatex' in self.dataset:
    ...
    def decode(self, frame_feats, cluster_feats, previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c):
        ...
        query = att_lstm_h.unsqueeze(1)
        for _, l in enumerate(self.layers_1):
            query = l(query, self.learnable_concept.expand(frame_feats.size(0),-1,-1))
        
        att_feat_text, weight_text = self.att_text(query, None, att_lstm_h)

        # language lstm decoder
        decoder_input = torch.cat([0.8*att_feat+0.2*att_feat_text, att_lstm_h], dim=1)
        # decoder_input = torch.cat([att_feat + att_feat_text, att_lstm_h], dim=1)
